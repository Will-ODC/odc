...
- [ ] **Require status checks to pass**, and **Require branches to be up to date** — mark required: `format / lint / typecheck`, `diff-size`, `guard-tests`, and `memory-index` (from `repo`), and `guard` (from `contracts-guard`). `guard-tests` must be required: it protects the guard scripts themselves, so a change that quietly defangs `contracts-guard.sh` can't merge on a still-green `guard`. `memory-index` fails when a top-level workstream directory has no row in `memory/INDEX.md` — the check that would have caught `apps/pulse` running nineteen PRs unrecorded. **Note the mechanism:** this is implemented as a **Ruleset**, not a classic branch rule — Settings → Rules → Rulesets, not Settings → Branches. A check must have run at least once before GitHub will offer it, and requiring a check that does not exist on master blocks every PR, so add the job first and require it after it has run green.
