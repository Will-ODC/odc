...
- **Non-equivocation by anchoring.** The chain's **identity** — the hash of its genesis event (`event-types.md` ET-7a) — and its current **head** are periodically published together where the operator cannot rewrite them: public repos in v1, anchored to an existing public blockchain in v2. Both halves are load-bearing. A head names a position on some chain, so publishing it alone lets an operator run two chains and anchor only one; publishing the identity is what makes two chains shown to two audiences visibly two (ADR-0013). This buys the one property blockchains genuinely offer, without running one: the log itself needs no consensus, because it has one writer and many verifiers.
...
- **Multiple aggregation methods in parallel** (approval, ranked-choice/STV, quadratic, others) computed from the same ballots. Choosing among them is a downstream act. _Roadmap, not a v1 property:_ a v1 ballot records **one choice from a small bounded set**, which yields plurality but neither ranking nor intensity — the richer ballots those methods need are a later, additive change. And they are **constrained, not merely deferred**: an expressive ballot is an identifying one, because a voter-chosen allocation across options has enough distinct values to serve as the covert marker a coercer can demand. Where expressiveness and receipt-freeness collide, **receipt-freeness wins** — §8 makes it non-negotiable and expressiveness carries no such protection (see also `event-types.md` ET-22, which bars an unbounded voter-chosen value permanently). Exactly where that ceiling sits is an open question, not settled here.
